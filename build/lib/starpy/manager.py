            ...
            if line:
                if line.endswith(self.END_DATA):
                    ...
                    if line.startswith(self.VERSION_PREFIX):
                        ...
                        log.debug('amiVersion: %s', self.amiVersion)
                    else:
                        try:
                            key, value = line.split(':', 1)
                            #log.debug('key: %s value: %s'%(key, value))
                            if (key.lower().strip() == 'output'):
                                # Kivko Vlad 2022 для Asterisk 17.5
                            #    message[0] = value.strip()
                                message.setdefault(' ', []).extend(
                                    [value for l in line.split('\n')
                                        if (value and value != '\n')]
                                )
